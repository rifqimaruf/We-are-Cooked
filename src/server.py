import socket
import threading
import json
import random
import time
import cProfile
import pstats
import signal

from src.shared.game_state import GameState
from src.shared import config

game_state = GameState()
connections = []
clients_info = {}
game_started = False
timer_thread_active = True
timer_thread_instance = None

server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
server.bind((config.SERVER_IP, config.SERVER_PORT))
server.listen()

print(f"RecipeManager initialized and recipes cached.")
print(f"Server listening on {config.SERVER_IP}:{config.SERVER_PORT}")

# Flag untuk memberi sinyal agar server berhenti
shutdown_flag = False # <<< TAMBAHKAN INI

def signal_handler(signum, frame): # <<< TAMBAHKAN FUNGSI INI
    global shutdown_flag
    print("\nCtrl+C detected. Initiating graceful shutdown...")
    shutdown_flag = True
    # Anda mungkin juga perlu menutup server socket agar accept() tidak memblokir
    server.close()

# Daftarkan signal handler ini di awal eksekusi utama
# (Setelah definisi fungsi dan variabel global)
# Ini akan dipanggil saat Ctrl+C (SIGINT) diterima
signal.signal(signal.SIGINT, signal_handler) # <<< TAMBAHKAN INI

# ... (definisi game_state, connections, server socket, dll) ...

def broadcast_state():
    state_dict = game_state.to_dict()
    # Add connected clients info to the state
    state_dict["clients_info"] = clients_info
    state_dict["game_started"] = game_started
    
    state_json = json.dumps(state_dict).encode()
    dead_conns = []
    for c in connections:
        try:
            c.sendall(state_json)
        except Exception as e:
            print(f"Error broadcasting to client: {e}")
            dead_conns.append(c)
    
    for dc in dead_conns:
        try:
            connections.remove(dc)
            dc.close()
        except:
            pass
    
    # print(f"Broadcast state: game_started={game_started}, players={len(game_state.players)}, clients={len(clients_info)}")

def restart_game():
    """Reset the game state and start a new timer thread"""
    global game_state, timer_thread_active, timer_thread_instance, game_started
    
    # Hentikan timer thread yang lama jika ada
    if timer_thread_instance and timer_thread_instance.is_alive():
        timer_thread_active = False # Memberi sinyal ke thread lama untuk berhenti
        timer_thread_instance.join(timeout=1.0) # Tunggu thread lama berhenti
        if timer_thread_instance.is_alive():
            print("Warning: Old timer thread did not terminate in time.")

    # Inisialisasi game_state yang baru
    # GameState() akan membuat instance baru dengan lock-nya sendiri
    game_state = GameState() 
    game_started = True
    print(f"Setting game_started to {game_started}")

    # Langkah 1: Hasilkan pesanan terlebih dahulu agar kita tahu bahan yang dibutuhkan
    num_connected_clients = len(clients_info)
    game_state.generate_orders(num_connected_clients) # Mengirim jumlah klien untuk filter resep
    print(f"Generated orders: {[o['name'] for o in game_state.orders]}")

    # Langkah 2: Kumpulkan semua bahan yang dibutuhkan oleh pesanan aktif
    required_ingredients_pool = []
    for order in game_state.orders:
        required_ingredients_pool.extend(order['ingredients'])

    random.shuffle(required_ingredients_pool)
    print(f"Required ingredients pool: {required_ingredients_pool}")

    all_possible_ingredients = [
        'Rice', 'Salmon', 'Tuna', 'Shrimp', 'Egg', 'Seaweed',
        'Cucumber', 'Avocado', 'Crab Meat', 'Eel', 'Cream Cheese', 'Fish Roe'
    ]

    # Daftar bahan yang akan diberikan ke setiap pemain.
    ingredients_to_assign = []
    temp_required_pool = list(required_ingredients_pool)

    for conn_index, conn in enumerate(connections):
        try:
            addr = conn.getpeername()
            player_id = str(addr)
            if player_id in clients_info:
                ingredient = None
                if temp_required_pool:
                    ingredient = temp_required_pool.pop(0)
                else:
                    ingredient = random.choice(all_possible_ingredients)

                ingredients_to_assign.append((player_id, ingredient))

        except Exception as e:
            print(f"Error preparing ingredient for player {player_id}: {e}")
            ingredients_to_assign.append((player_id, random.choice(all_possible_ingredients)))
            continue

    random.shuffle(ingredients_to_assign)

    # Langkah 4: Tambahkan pemain ke game state dengan bahan dan posisi yang ditugaskan
    for player_id, ingredient in ingredients_to_assign:
        pos = (random.randint(0, config.GRID_WIDTH - 1), random.randint(0, config.GRID_HEIGHT - 1))
        game_state.add_player(player_id, ingredient, pos) # add_player sudah pakai lock internal
        print(f"Added player {player_id} as {ingredient} to game state at {pos}")

    # Mulai timer baru
    timer_thread_active = True
    timer_thread_instance = threading.Thread(target=timer_thread, daemon=True)
    timer_thread_instance.start()

    broadcast_state() # broadcast_state akan memanggil to_dict, yang sudah dilindungi lock internal

    print("Game started with new state")

def return_to_lobby():
    """Return all players to the lobby without restarting the game"""
    global game_started, game_state, timer_thread_active, timer_thread_instance

    # Hentikan timer thread yang lama jika ada
    if timer_thread_instance and timer_thread_instance.is_alive():
        timer_thread_active = False
        timer_thread_instance.join(timeout=1.0)
        if timer_thread_instance.is_alive():
            print("Warning: Old timer thread did not terminate for lobby return.")

    # Reset game state
    game_started = False

    # Reset all players' ready status
    for player_id in clients_info:
        clients_info[player_id]["ready"] = False

    # Create a fresh game state but don't start the timer
    game_state = GameState() # Membuat GameState baru dengan lock-nya sendiri

    broadcast_state() # broadcast_state akan memanggil to_dict, yang sudah dilindungi lock internal
    print("All players returned to lobby, game state reset")

def handle_client(conn, addr):
    print(f"[NEW CONNECTION] {addr} connected.")
    connections.append(conn)

    player_id = str(addr)
    default_username = f"Player{addr[1] % 1000}"
    clients_info[player_id] = {"username": default_username, "ready": False}

    # Kirim state awal ke klien baru
    broadcast_state() # Ini mengirimkan state_dict, clients_info, dan game_started

    try:
        # Kirim ID klien spesifik setelah broadcast state awal
        # Ini mungkin menyebabkan duplikasi pengiriman state jika broadcast_state juga mengirimkannya
        # Lebih baik broadcast_state sudah mengirimkan client_id dan client_info yang relevan
        # Mari kita asumsikan broadcast_state sudah cukup.
        # conn.sendall(json.dumps({"client_id": player_id, **game_state.to_dict(), 
        #                         "clients_info": clients_info, 
        #                         "game_started": game_started}).encode())

        while True:
            data = conn.recv(config.BUFFER_SIZE)
            if not data:
                break

            msg = json.loads(data.decode())

            if msg.get("action") == "set_username":
                username = msg.get("username", default_username)
                clients_info[player_id]["username"] = username
                print(f"Player {player_id} set username to {username}")
                broadcast_state()

            elif msg.get("action") == "toggle_ready":
                clients_info[player_id]["ready"] = not clients_info[player_id].get("ready", False)
                ready_status = "ready" if clients_info[player_id]["ready"] else "not ready"
                print(f"Player {player_id} toggled ready status to {ready_status}")
                broadcast_state()

            elif msg.get("action") == "start_game" and not game_started:
                all_ready = all(client["ready"] for client in clients_info.values())
                print(f"Start game requested by {player_id}. All players ready: {all_ready}")                
                if all_ready and len(clients_info) > 0:
                    restart_game()

            elif msg.get("action") == "return_to_lobby":
                return_to_lobby()

            elif msg.get("action") == "move" and game_started:
                direction = msg["direction"]
                game_state.move_player(player_id, direction)
                # <<< HAPUS broadcast_state() DI SINI >>>
                # broadcast_state() # Akan terlalu sering, ditangani oleh timer_thread

                # <<< HAPUS check_for_merge() DI SINI >>>
                # fusion_result = game_state.check_for_merge() # Akan terlalu sering, ditangani oleh timer_thread
                # if fusion_result:
                #     print(f"Fusion at {fusion_result['pos']}: {fusion_result['fusion']['name']} served!")

            elif msg.get("action") == "restart":
                print(f"Restart requested by {addr}")
                restart_game()

    except Exception as e:
        print(f"[ERROR] Connection with {addr} ended unexpectedly: {e}")
    finally:
        if player_id in clients_info:
            del clients_info[player_id]
        game_state.remove_player(player_id)
        if conn in connections:
            connections.remove(conn)
        broadcast_state()
        conn.close()
        print(f"[DISCONNECT] {addr} disconnected.")

# Tambahkan global variable untuk mengontrol frekuensi check_for_merge
last_merge_check_time = 0 
MERGE_CHECK_INTERVAL = 0.1 # Cek merge setiap 0.1 detik (10 kali per detik)

def timer_thread():
    global timer_thread_active, game_started, last_merge_check_time

    start_time = time.time()
    game_duration = game_state.timer
    end_time = start_time + game_duration

    broadcast_interval = 0.05 # <<< UBAH INI (Sangat sering, 20x per detik)
    next_broadcast_time = start_time

    # MERGE_CHECK_INTERVAL kita pertahankan, itu adalah frekuensi logika fusi
    # Tapi broadcast akan lebih sering.

    while timer_thread_active:
        current_time = time.time()
        remaining = max(0, end_time - current_time)

        game_state.timer = int(remaining) 

        # Cek merge secara teratur
        if game_started and current_time - last_merge_check_time >= MERGE_CHECK_INTERVAL:
            fusion_result = game_state.check_for_merge()
            if fusion_result:
                # Jika ada fusi, langsung broadcast state terbaru
                broadcast_state() 
                next_broadcast_time = current_time + broadcast_interval # Reset broadcast timer
            last_merge_check_time = current_time

        # Broadcast state ke klien dengan interval yang lebih cepat
        if current_time >= next_broadcast_time:
            broadcast_state() 
            next_broadcast_time = current_time + broadcast_interval

        if remaining <= 0:
            game_state.timer = 0
            game_started = False
            broadcast_state()
            print(f"Game Over! Final Score: {game_state.score}")
            break

        # Tidur sangat sebentar untuk memberikan kesempatan CPU ke thread lain
        time.sleep(0.001) # <<< UBAH INI (Sangat kecil, 1000x per detik)

def start():
    global timer_thread_instance, game_started, shutdown_flag # Pastikan shutdown_flag global
    game_started = False

    # Loop utama server, akan berhenti saat shutdown_flag True
    while not shutdown_flag:
        try:
            # Menggunakan timeout agar server.accept() tidak memblokir selamanya
            # dan bisa memeriksa shutdown_flag
            server.settimeout(0.5) # Timeout 0.5 detik
            conn, addr = server.accept()
            server.settimeout(None) # Reset timeout setelah koneksi diterima

            if shutdown_flag: # Cek lagi setelah accept
                break

            thread = threading.Thread(target=handle_client, args=(conn, addr), daemon=True)
            thread.start()
        except socket.timeout:
            # Ini normal jika tidak ada koneksi baru selama timeout
            pass
        except Exception as e:
            if not shutdown_flag: # Jangan cetak error jika sudah dalam proses shutdown
                print(f"Error in server.accept(): {e}")
            break # Keluar dari loop jika ada error fatal atau shutdown

if __name__ == "__main__":
    # Jalankan start() dengan profiler
    profiler = cProfile.Profile()
    profiler.enable() # Mulai mengumpulkan data profil

    try:
        start() # Panggil fungsi utama server
    except KeyboardInterrupt:
        print("\nServer shutting down...")
    except Exception as e:
        print(f"Server crashed: {e}")
    finally:
        profiler.disable() # Hentikan pengumpulan data

        # Simpan hasil profil ke file
        stats_file = "server_profile_stats.prof"
        profiler.dump_stats(stats_file)
        print(f"Profiling data saved to {stats_file}")

        # Tampilkan ringkasan hasil profil di konsol
        stats = pstats.Stats(stats_file)
        stats.sort_stats(pstats.SortKey.TIME) # Urutkan berdasarkan total waktu
        stats.print_stats(20) # Tampilkan 20 fungsi teratas yang paling banyak memakan waktu
