...
import requests
import logging
from src.shared import config # Memastikan import config dari shared
...
class HttpNetworkHandler:
    ...
    def __init__(self, game_manager): # Hapus parameter 'port=None'
        self.game_manager = game_manager
        self.lobby_url = f"http://{config.LOBBY_SERVER_IP}:{config.LOBBY_SERVER_PORT}"
        self.game_server_url = None # URL Game Server akan didapat dari Lobby Server
        self.client_id = None
        self.thread = None
        self.running = False
    ...
    def start(self):
        ...
        try:
            ...
            response = self.session.post(
                f"{self.game_server_url}/connect", # Kirim action connect ke game server
                json={"action": "connect"},
                timeout=5.0
            )
    ...
    def stop(self):
        ...
        if self.client_id and self.game_server_url: # Pastikan ada game_server_url
            try:
                self.session.post(
                    f"{self.game_server_url}/disconnect", # Kirim ke game_server_url
